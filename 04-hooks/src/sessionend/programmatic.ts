...
import { validateSessionEnd, printValidation } from "../shared/validate.ts";
...
export async function runProgrammatic(): Promise<string> {
  ...
  const validation = validateSessionEnd(logData);
  printValidation(validation, "SessionEndHookInput");
