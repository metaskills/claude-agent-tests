...
import { validatePostToolUse, printValidation } from "../shared/validate.ts";
...
export async function runProgrammatic(): Promise<string> {
  ...
  const validation = validatePostToolUse(logData);
  printValidation(validation, "PostToolUseHookInput");
