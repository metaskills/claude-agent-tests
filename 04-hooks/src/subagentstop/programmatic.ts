...
import { validateSubagentStop, printValidation } from "../shared/validate.ts";
...
export async function runProgrammatic(): Promise<string> {
  ...
  const validation = validateSubagentStop(logData);
  printValidation(validation, "SubagentStopHookInput");
